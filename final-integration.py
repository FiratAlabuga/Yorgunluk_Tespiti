...
alert = vlc.MediaPlayer('ses_tr.mp3')
alert2 = vlc.MediaPlayer('ses_en.mp3')
...
while(True):
    ...
    if(len(rects)):
        ...
        if(yawn(shape[mStart:mEnd])>0.6):
            cv2.putText(gray, "Esneme Algilandi", (50,50), cv2.FONT_HERSHEY_COMPLEX, 1,(0,255,127),2)
            yawn_countdown=1
        ...
        if(avgEAR<close_thresh):
            ...
            if(yawn_countdown and flag>=frame_thresh_3):
                ...
                cv2.putText(gray, "Esnemeden sonra uykulu", (50,50), cv2.FONT_HERSHEY_COMPLEX, 1,(0,255,127),2)
                alert.play()
                alert2.play()
                if(map_flag):
                    map_flag = 0
                ...
                cv2.putText(gray, "Uykulu (Vucut Durusu)", (50,50), cv2.FONT_HERSHEY_COMPLEX, 1,(0,255,127),2)
                alert.play()
                alert2.play()
                if(map_flag):
                    map_flag = 0
                ...
                cv2.putText(gray, "Uykulu (Normal)", (50,50), cv2.FONT_HERSHEY_COMPLEX, 1,(0,255,127),2)
                alert.play()
                alert2.play()
                if(map_flag):
                    map_flag = 0
        ...
        if(map_counter>=3):
            map_flag=1
            map_counter=0
            cv2.imwrite(os.path.join(path,'sofor.jpg'),frame)
            vlc.MediaPlayer('ses_tr_araver.mp3').play()
            mail_gonder.SendMail("sofor.jpg")
            webbrowser.open("https://www.google.com/maps/search/hotels+or+motels+near+adana")
    ...
    if(avgEAR>close_thresh):
        alert.stop()
    cv2.imshow('Surucu Izleme Panel', gray)
    if(cv2.waitKey(1)==27):
        break
